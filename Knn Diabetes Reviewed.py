...
#Hossein - The line here on 52nd was redundant and got removed
...
#as far as I know does BloodPressure not influence if you have diabetes or not
cleanDF.head()
rawDF['Outcome'] = rawDF['Outcome'].replace([0]) #Hossein - Removed the letters D and N Changed the place of N and D in this and next line.
rawDF['Outcome'] = rawDF['Outcome'].replace([1]) 
#the D stands for a diagnosis of diabetes
#the N stands fot a diagnosis of no diabetes

## Data Preparation
cntDiag = testDF['Outcome'].value_counts() #Hossein - cleanDF changed to testDF
propDiag = testDF['Outcome'].value_counts(normalize=True) #Hossein - cleanDF changed to testDF
cntDiag #the outcome is in this case the same as the diagnosis, so to see if someone has diabetes or not
propDiag 

cleanDF.info()

catType = CategoricalDtype(categories=[0,1], ordered=False) #Hossein added this line and the next two were missing
...
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score #Hossein - to better evaluate the performance of the KNN model, I calculated and printed some metrics such as accuracy, precision, recall, and F1 score
...
plt.show() #Hossein - Added

## Evaluation and Deployment

#Hossein's side note 
The final plot is a confusion matrix that shows the number of true positives, false positives, true negatives, and false negatives for the predictions made by the KNN model.
#---------------------------------------------------------------
